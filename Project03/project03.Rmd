...
def GibbsMotifFinder (seqs, k, seed=42):
    ...
    while ((j < 1000) and (counter_ic < 10)): 
      ...
      exp_scores = [math.exp(score) for score in poss_motif_scores] 
      rand_index = choose_motif(poss_motifs, exp_scores) # probabilistic motif selection
      pfm = build_pfm(poss_motifs[rand_index], k)  # build pfm from the probabilistic selected motif
      ...
      if (fuzzy_diff (current_ic,old_ic,0.000001)):  #if the difference is minimal, add 1 to counter
        ...
        counter_ic = 0
      print(f"J: {j}, PFM:{pfm} , Old IC: {old_ic}, Current IC: {current_ic}, IC Counter: {counter_ic}")
