...
def choose_motif(motif_list, weights):
  ...
  args:
    ...
    weights (list of floats): list of probailities of the scores associated with the motifs.
  '''
  if (motif_list):
...
def GibbsMotifFinder (seqs, k, seed=42):
    ...
    # Loop converges once either counter has reached its condition
    while ((j < 10000) and (counter_ic < 100)): 
      ...
      all_motifs[N] = selected_motif
      
      # remove at the end
      motif = []  
      motif.append(selected_motif)
      
      #pfm = build_pfm(motif, k)  # build pfm from the probabilistic selected motif
      pfm = build_pfm(all_motifs, k)      
      current_ic = pfm_ic(pfm)
            
      if (fuzzy_diff (current_ic,old_ic, 0.005)):  #if the difference is minimal, add 1 to counter
        counter_ic += 1
      else:
        counter_ic = 0
    ...
    print(f"Final motif: {motif}, PFM: {pfm}")
    return (np.array(pfm))
